...
\n\nAdd "ipv6.disable=1" to the boot option, then save your grub file:\n\nGRUB_CMDLINE_LINUX_DEFAULT="ipv6.disable=1 quiet splash"\n\nsudo update-grub\n\nFor more details You could visit:http://askubuntu.com/questions/309461/how-to-disable-ipv6-permanently\n\n######################\n\n' >> $LOG
    ...
    V-38634)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must rotate audit log files that reach the maximum file size.' 
              else
                  log_msg $2 '该系统必须有转动，达到最大文件大小审核日志文件。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-54381)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must switch the system to single-user mode when available audit storage volume becomes dangerously low.' 
              else
                  log_msg $2 '当可用的审计存储体积变得严重不足时审计系统必须切换至单用户模式。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38635)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all attempts to alter system time through adjtimex.' 
              else
                  log_msg $2 '审计系统必须配置审核所有试图通过adjtimex修改系统时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38522)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all attempts to alter system time through settimeofday.' 
              else
                  log_msg $2 '审计系统必须配置审核所有试图通过settimeofday修改系统时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38525)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all attempts to alter system time through stime.' 
              else
                  log_msg $2 '审计系统必须配置审核所有试图通过STIME修改系统时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38527)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all attempts to alter system time through clock_settime.' 
              else
                  log_msg $2 '审计系统必须配置审核所有试图通过clock_gettime修改系统时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38530)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all attempts to alter system time through /etc/localtime.' 
              else
                  log_msg $2 '审计系统必须配置审核所有试图通过/etc/localtime对系统时间进行改变的行为。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38531)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must automatically audit account creation.' 
              else
                  log_msg $2 '操作系统必须自动审核帐户创建。'
              fi
              if [ $2 -ne 0 ];then 
...
system administrator(s) to any modifications. Any unexpected users, groups, or modifications should be investigated for legitimacy.\n\nFix text: Add the following to "/etc/audit/audit.rules", in order to capture events that modify account changes:\n\n# audit_account_changes\n-w /etc/group -p wa -k audit_account_changes\n-w /etc/passwd -p wa -k audit_account_changes\n-w /etc/gshadow -p wa -k audit_account_changes\n-w /etc/shadow -p wa -k audit_account_changes\n-w /etc/security/opasswd -p wa -k audit_account_changes\n\n######################\n\n' >> $LOG
    ...
    V-38534)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must automatically audit account modification.' 
              else
                  log_msg $2 '操作系统必须自动审核帐户的修改。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38536)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must automatically audit account disabling actions.' 
              else
                  log_msg $2 '操作系统必须自动审核帐号禁用的行为。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38538)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must automatically audit account termination.' 
              else
                  log_msg $2 '操作系统必须自动审核帐号的终止行为'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38540)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit modifications to the systems network configuration.' 
              else
                  log_msg $2 '审核系统必须配置对系统网络配置的自动审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38541)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured audit modifications to the systems Mandatory Access Control (MAC) configuration (Apparmor).' 
              else
                  log_msg $2 '审核系统必须配置对强制访问控制（Apparmor）配置的修改的审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38543)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using chmod.' 
              else
                  log_msg $2 '审核系统必须配置对通过chmod命令进行访问控制权限的修改行为进行审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38545)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using chown.' 
              else
                  log_msg $2 '审核系统必须配置对通过chown命令修改属主关系的所有行为进行审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38547)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fchmod.' 
              else
                  log_msg $2 '审核系统必须配置对通过fchmod修改访问控制权限的所有行为进行审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38550)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fchmodat.' 
              else
                  log_msg $2 '审核系统必须配置对使用fchmodat进行访问控制权限的修改的所有行为进行审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38552)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fchown.' 
              else
                  log_msg $2 '审核系统必须配置对使用fchown进行访问控制权限的修改的所有行为进行审核。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38554)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fchownat.' 
              else
                  log_msg $2 '审计系统必须配置对使用fchownat修改访问控制权限的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38556)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fremovexattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用fremovexattr修改访问控制权限的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38557)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using fsetxattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用fsetxattr进行修改访问控制权限的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38558)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using lchown.' 
              else
                  log_msg $2 '审计系统必须配置对使用lchown进行访问控制权限的修改的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38559)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using lremovexattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用lremovexattr进行访问控制权限的修改的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38561)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using lsetxattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用lsetxattr进行访问控制权限的修改的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38563)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using removexattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用removexattr进行访问控制权限的修改的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38565)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all discretionary access control permission modifications using setxattr.' 
              else
                  log_msg $2 '审计系统必须配置对使用setxattr进行访问控制权限的修改的所有行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38566)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit failed attempts to access files and programs.' 
              else
                  log_msg $2 '审计系统必须配置失败的尝试访问文件或程序的行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38567)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit all use of setuid and setgid programs.' 
              else
                  log_msg $2 '审计系统必须配置对所有的setuid和setgid的程序进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38568)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit successful file system mounts.' 
              else
                  log_msg $2 '审计系统必须配置对成功进行文件系统的挂载进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38575)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit user deletions of files and programs.' 
              else
                  log_msg $2 '审计系统必须配置对用户删除文件或程序的行为进行审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38578)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit changes to the /etc/sudoers file.' 
              else
                  log_msg $2 '审计系统必须配置对文件/etc/sudoers文件的修改的审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38580)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must be configured to audit the loading and unloading of dynamic kernel modules.' 
              else
                  log_msg $2 '审计系统必须配置对装载与卸载动态内核模块的审计。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38582)  if [ "$3" = "en" ]; then
                  log_msg $2 'The xinetd service must be disabled if no network services utilizing it are enabled.' 
              else
                  log_msg $2 '如果没有网络服务已启用xinetd服务，则此服务必须被禁止。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38584)  if [ "$3" = "en" ]; then
                  log_msg $2 'The xinetd service must be uninstalled if no network services utilizing it are enabled.' 
              else
                  log_msg $2 '如果没有网络服务已启用xinetd服务，则xinetd服务必须未安装。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38587)  if [ "$3" = "en" ]; then
                  log_msg $2 'The telnet-server package must not be installed.' 
              else
                  log_msg $2 'telnet服务软件包必须没有进行安装。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38589)  if [ "$3" = "en" ]; then
                  log_msg $2 'The telnet daemon must not be running.' 
              else
                  log_msg $2 'telnet守护程序必须没有运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38591)  if [ "$3" = "en" ]; then
                  log_msg $2 'The rsh-server package must not be installed.' 
              else
                  log_msg $2 'rsh服务软件包必须没有进行安装。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38594)  if [ "$3" = "en" ]; then
                  log_msg $2 'The rshd service must not be running.' 
              else
                  log_msg $2 'rshd服务必须没有运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38598)  if [ "$3" = "en" ]; then
                  log_msg $2 'The rexecd service must not be running.' 
              else
                  log_msg $2 'rexecd服务必须没有运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38602)  if [ "$3" = "en" ]; then
                  log_msg $2 'The rlogind service must not be running.' 
              else
                  log_msg $2 'rlogind服务必须没有进行运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38603)  if [ "$3" = "en" ]; then
                  log_msg $2 'The nis(ypserv) package must not be installed.' 
              else
                  log_msg $2 'nis(ypserv)安装包必须没有安装。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38604)  if [ "$3" = "en" ]; then
                  log_msg $2 'The nis(ypbind) service must not be running.' 
              else
                  log_msg $2 'nis(ypserv)服务必须没有运行。'
              fi
              if [ $2 -ne 0 ];then 
...
disabled. The "nis" service can be disabled with the following commands:\n\n#update-rc.d nis remove\nservice nis stop\n\n######################\n\n' >> $LOG
    ...
    V-38606)  if [ "$3" = "en" ]; then
                  log_msg $2 'The tftp-server package must not be installed unless required.' 
              else
                  log_msg $2 'tftp服务软件包必须没有安装，除非必需。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38609)  if [ "$3" = "en" ]; then
                  log_msg $2 'The tftp service must not be running.' 
              else
                  log_msg $2 'tftp服务必须没有运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38605)  if [ "$3" = "en" ]; then
                  log_msg $2 'The cron service must be running.' 
              else
                  log_msg $2 'cron服务必须正常运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38607)  if [ "$3" = "en" ]; then
                  ...
                  log_msg $2 'SSH守护进程必须配置为仅支持SSHv2协议。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38608)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must set a timeout interval on idle sessions.' 
              else
                  log_msg $2 'SSH守护进程必须设置空闲会话的超时间隔时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38610)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must set a timeout count on idle sessions.' 
              else
                  log_msg $2 'SSH守护进程必须设置空闲会话的超时数。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38611)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must ignore .rhosts files.' 
              else
                  log_msg $2 'SSH守护进程必须忽略.rhosts文件。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38612)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must not allow host-based authentication.' 
              else
                  log_msg $2 'SSH守护进程必须不允许基于主机的认证。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38613)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must not permit root logins using remote access programs such as ssh.' 
              else
                  log_msg $2 '必须不允许类似ssh的远程登录程序使用root帐号进行远程登录。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38614)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must not allow authentication using an empty password.' 
              else
                  log_msg $2 'SSH守护进程必须不允许通过一个空密码进行认证。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38616)  if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must not permit user environment settings.' 
              else
                  log_msg $2 'SSH守护进程必须不允许进行用户环境的设置。'
              fi
              if [ $2 -ne 0 ];then 
     ...
     V-38617) if [ "$3" = "en" ]; then
                  log_msg $2 'The SSH daemon must be configured to use only FIPS 140-2 approved ciphers.' 
              else
                  log_msg $2 'SSH守护进程必须配置为使用满足FIPS 140-2标准认可的密码。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38618)  if [ "$3" = "en" ]; then
                  log_msg $2 'The avahi service must be disabled.' 
              else
                  log_msg $2 'avahi服务必须设置为失效。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38620)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system clock must be synchronized continuously, or at least daily.' 
              else
                  log_msg $2 '系统时钟必须至少每天同步。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38621)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system clock must be synchronized to an authoritative trusted time source.' 
              else
                  log_msg $2 '系统时钟必须同步权威可信时间源。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38622)  if [ "$3" = "en" ]; then
                  log_msg $2 'Mail relaying must be restricted.' 
              else
                  log_msg $2 '邮件中继必须加以限制。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38625)  if [ "$3" = "en" ]; then
                  log_msg $2 'If the system is using LDAP for authentication or account information, the system must use a TLS connection using FIPS 140-2 approved cryptographic algorithms.' 
              else
                  log_msg $2 '如果系统使用了LDAP认证或帐号信息，系统必须使用满足140-2的密码算法的TLS进行连接。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38626)  if [ "$3" = "en" ]; then
                  log_msg $2 'The LDAP client must use a TLS connection using trust certificates signed by the site CA.' 
              else
                  log_msg $2 'LDAP客户端必须使用由CA签发的认证证书进行TLS连接。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38627)  if [ "$3" = "en" ]; then
                  log_msg $2 'The openldap-servers package must not be installed unless required.' 
              else
                  log_msg $2 'Openldap服务软件包必须没有进行安装，除非必需。'
              fi
              if [ $2 -ne 0 ];then 
...
system.\n\nFix text: The "sldap" package should be removed if not in use. Is this machine the OpenLDAP server? If not, remove the package.\n\n#apt-get purge sldap\n\nThe openldap-servers RPM is not installed by default on RHEL6 machines. It is needed only by the OpenLDAP server, not by the clients which use LDAP for authentication. If the system is not intended for use as an LDAP Server it should be removed.  \n\n######################\n\n' >> $LOG
    ...
    V-38629)  if [ "$3" = "en" ]; then
                  log_msg $2 'The graphical desktop environment must set the idle timeout to no more than 15 minutes.' 
              else
                  log_msg $2 '图形桌面环境必须设置空闲超时时间不大于15分钟。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38630)  if [ "$3" = "en" ]; then
                  log_msg $2 'The graphical desktop environment must automatically lock after 15 minutes of inactivity and the system must require user reauthentication to unlock the environment.'
              else
                  log_msg $2 '图形化桌面环境必须闲置15分钟后自动锁定，系统必须要求用户重新认证解锁环境。'
              fi 
              if [ $2 -ne 0 ];then 
    ...
    V-38638)  if [ "$3" = "en" ]; then
                  log_msg $2 'The graphical desktop environment must have automatic lock enabled.' 
              else
                  log_msg $2 '图形化桌面环境必须自动锁定已经开启。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38639)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must display a publicly-viewable pattern during a graphical desktop environment session lock.' 
              else
                  log_msg $2 '系统必须图形桌面环境会话锁定时显示一个公开展示的模式。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38641)  if [ "$3" = "en" ]; then
                  log_msg $2 'The atd service must be disabled.' 
              else
                  log_msg $2 'atd服务必须设置为失效。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38652)  if [ "$3" = "en" ]; then
                  log_msg $2 'Remote file systems must be mounted with the nodev option.' 
              else
                  log_msg $2 '远程文件系统必须使用nodev选项进行挂载。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38654)  if [ "$3" = "en" ]; then
                  log_msg $2 'Remote file systems must be mounted with the nosuid option.' 
              else
                  log_msg $2 '远程文件系统必须使用nosuid选项进行挂载。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38655)  if [ "$3" = "en" ]; then
                  log_msg $2 'The noexec option must be added to removable media partitions.' 
              else
                  log_msg $2 'noexec选项必须在挂载可移动分区时使用。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38656)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must use SMB client signing for connecting to samba servers using smbclient' 
              else
                  log_msg $2 '系统必须使用SMB客户端签名，用于连接使用smbclient的samba服务器。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38657)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must use SMB client signing for connecting to samba servers using mount.cifs.' 
              else
                  log_msg $2 '系统必须使用SMB客户端签名使用mount.cifs配置去连接samba服务器。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38658)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must prohibit the reuse of passwords within twenty-four iterations.' 
              else
                  log_msg $2 "系统必须禁止重用最近24次的密码。"
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38659)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must employ cryptographic mechanisms to protect information in storage.' 
              else
                  log_msg $2 '操作系统必须采用加密机制来保护信息存储。'
              fi
              if [ $2 -ne 0 ];then 
...
site:\nhttps://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html  \n\n######################\n\n' >> $LOG
    ...
    V-38661)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must protect the confidentiality and integrity of data at rest.' 
              else
                  log_msg $2 '操作系统必须保护静态数据的机密性和完整性。'
              fi
              if [ $2 -ne 0 ];then 
...
site:\nhttps://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html  \n\n######################\n\n' >> $LOG
    ...
    V-38662)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must employ cryptographic mechanisms to prevent unauthorized disclosure of data at rest unless otherwise protected by alternative physical measures.' 
              else
                  log_msg $2 '操作系统必须采用加密机制，以防止数据的未经授权的数据泄漏，除非通过其他物理措施进行保护。'
              fi
              if [ $2 -ne 0 ];then 
...
site:\nhttps://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html  \n\n######################\n\n' >> $LOG
    ...
    V-38663)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system package management tool must verify permissions on all files and directories associated with the audit package.' 
              else
                  log_msg $2 '系统软件包管理工具必须验证与审核包相关的所有文件和目录的权限。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38664)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system package management tool must verify ownership on all files and directories associated with the audit package.' 
              else
                  log_msg $2 '该系统软件包管理工具必须验证与审核包关联的所有文件和目录的所有权。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38665)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system package management tool must verify group-ownership on all files and directories associated with the audit package.' 
              else
                  log_msg $2 '系统软件包管理工具必须验证与审核包关联的所有文件和目录的组所有权。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38637)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system package management tool must verify contents of all files associated with the audit package.' 
              else
                  log_msg $2 '系统软件包管理工具必须验证与审核程序包关联的所有文件的内容。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38668)  if [ "$3" = "en" ]; then
                  log_msg $2 'The x86 Ctrl-Alt-Delete key sequence must be disabled.' 
              else
                  log_msg $2 'X86的组合快捷键Ctrl-Alt-Delete必须使其失效。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38669)  if [ "$3" = "en" ]; then
                  log_msg $2 'The postfix service must be enabled for mail delivery.' 
              else
                  log_msg $2 'postfix邮件服务必须是开启的。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38671)  if [ "$3" = "en" ]; then
                  log_msg $2 'The sendmail package must be removed.' 
              else
                  log_msg $2 'sendmail软件包必须删除。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38674)  if [ "$3" = "en" ]; then
                  log_msg $2 'X Windows must not be enabled unless required.' 
              else
                  log_msg $2 'X窗口程序必须处于未开启状态，除非必需。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38676)  if [ "$3" = "en" ]; then
                  log_msg $2 'The xorg-x11-server-common (X Windows) package must not be installed, unless required.' 
              else
                  log_msg $2 'xorg-x11-server-common软件包必须没有进行安装，除非必需。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38681)  if [ "$3" = "en" ]; then
                  log_msg $2 'All GIDs referenced in /etc/passwd must be defined in /etc/group' 
              else
                  log_msg $2 '在/etc/passwd中的所有GID引用的定义必须在/etc/group文件中。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38683)  if [ "$3" = "en" ]; then
                  log_msg $2 'All accounts on the system must have unique user or account names' 
              else
                  log_msg $2 '系统中的所有帐号必须具有独一无二的用户名或帐户名。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38693)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must require passwords to contain no more than three consecutive repeating characters.' 
              else
                  log_msg $2 "系统必须要求密码包含不超过3个连续重复的字符。"
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38695)  if [ "$3" = "en" ]; then
                  log_msg $2 'A file integrity tool must be used at least weekly to check for unauthorized file changes, particularly the addition of unauthorized system libraries or binaries, or for unauthorized modification to authorized system libraries or binaries.'
              else
                  log_msg $2 '一个文件完整性工具必须至少每周用于检查未经授权的文件的变化，特别是除了未经授权的系统库或二进制文件，或未经授权的修改授权的系统库或二进制文件。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38696)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must employ automated mechanisms, per organization defined frequency, to detect the addition of unauthorized components/devices into the operating system.' 
              else
                  log_msg $2 '操作系统必须采用自动化机制，每个组织定义的频率，以检测除了未授权的组件/装置的运作系统。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38698)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must employ automated mechanisms to detect the presence of unauthorized software on organizational information systems and notify designated organizational officials in accordance with the organization defined frequency.' 
              else
                  log_msg $2 '操作系统必须采用自动化的机制来检测未经授权的软件对组织信息系统的存在，并根据组织定义的频率通知指定的组织的官员。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38675)  if [ "$3" = "en" ]; then
                  log_msg $2 'Process core dumps must be disabled unless needed.' 
              else
                  log_msg $2 '处理核心转储必须被禁止，除非需要。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38677)  if [ "$3" = "en" ]; then
                  log_msg $2 'The NFS server must not have the insecure file locking option enabled.' 
              else
                  log_msg $2 'NFS服务器必须尚未启用不安全的文件锁定选项。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38680)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must identify staff members to receive notifications of audit log storage volume capacity issues.' 
              else
                  log_msg $2 '审计系统必须查明工作人员收到的审计日志存储卷容量问题的通知。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38682)  if [ "$3" = "en" ]; then
                  log_msg $2 'The Bluetooth kernel module must be disabled.' 
              else
                  log_msg $2 '蓝牙内核模块必须设置为失效。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38684)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must limit users to 1 simultaneous system logins, or a site-defined number, in accordance with operational requirements.' 
              else
                  log_msg $2 '系统必须禁止多个用户共用一个帐号。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38686)  if [ "$3" = "en" ]; then
                  log_msg $2 'The systems local firewall must implement a deny-all, allow-by-exception policy for forwarded packets.' 
              else
                  log_msg $2 '系统本地防火墙必须实现转发的数据包一个全部拒绝，允许按例外策略。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38691)  if [ "$3" = "en" ]; then
                  log_msg $2 'The Bluetooth service must be disabled.' 
              else
                  log_msg $2 '蓝牙服务必须设置为失效。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38692)  if [ "$3" = "en" ]; then
                  log_msg $2 'Accounts must be locked upon 35 days of inactivity.' 
              else
                  log_msg $2 "帐户必须在35天不使用后被锁定。"
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38694)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must manage information system identifiers for users and devices by disabling the user identifier after an organization defined time period of inactivity.' 
              else
                  log_msg $2 '操作系统必须通过禁止不活动的组织定义的时间段之后的用户标识符管理用户和设备的信息系统标识符。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38697)  if [ "$3" = "en" ]; then
                  log_msg $2 'The sticky bit must be set on all public directories.' 
              else
                  log_msg $2 'sticky位必须为所有的公共目录进行设置。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38701)  if [ "$3" = "en" ]; then
                  log_msg $2 'The TFTP daemon must operate in secure mode which provides access only to a single directory on the host file system.' 
              else
                  log_msg $2 '在TFTP守护程序必须在其中只对主机文件系统上的单个目录提供了访问的安全模式下运行。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38702)  if [ "$3" = "en" ]; then
                  log_msg $2 'The FTP daemon must be configured for logging or verbose mode.' 
              else
                  log_msg $2 '该FTP守护进程必须配置为记录或详细模式。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38645)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system default umask in /etc/login.defs must be 077.' 
              else
                  log_msg $2 '在/etc/login.defs中系统默认的umask必须是077。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38642)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system default umask for daemons must be 027 or 022.' 
              else
                  log_msg $2 '系统默认的umask必须设置为027或022。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38619)  if [ "$3" = "en" ]; then
                  log_msg $2 'There must be no .netrc files on the system.' 
              else
                  log_msg $2 '在系统中必须不存在.netrc文件。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38592)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must require administrator action to unlock an account locked by excessive failed login attempts.' 
              else
                  log_msg $2 "系统必须要求系统管理员在多次登录尝试失败时对账号进行锁定。"
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-51875)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system, upon successful logon/access, must display to the user the number of unsuccessful logon/access attempts since the last successful logon/access.' 
              else
                  log_msg $2 '操作系统，在成功登录/访问，必须自上次成功登录/接入向用户显示失败的登录/访问尝试的次数。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38498)  if [ "$3" = "en" ]; then
                  log_msg $2 'Audit log files must have mode 0640 or less permissive.' 
              else
                  log_msg $2 '审计日志文件的访问权限必须设置为root用户可读写、同组用户可读的权限或更小的权限。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38495)  if [ "$3" = "en" ]; then
                  log_msg $2 'Audit log files must be owned by root.' 
              else
                  log_msg $2 '审计日志文件的属主必须为root用户。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38493)  if [ "$3" = "en" ]; then
                  log_msg $2 'Audit log directories must have mode 0755 or less permissive.' 
              else
                  log_msg $2 '审计日志的存放目录的权限设置必须为root用户具有读写执行的权限、同组用户可读可执行、其他用户具有可读可执行的权限或更小的权限。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38490)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system must enforce requirements for the connection of mobile devices to operating systems.' 
              else
                  log_msg $2 '操作系统必须执行用于移动设备的对操作系统的连接要求。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38484)  if [ "$3" = "en" ]; then
                  log_msg $2 'The operating system, upon successful logon, must display to the user the date and time of the last logon or access via ssh.' 
              else
                  log_msg $2 '操作系统，在成功登录，必须向用户显示通过SSH上次登录或访问的日期和时间。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38471)  if [ "$3" = "en" ]; then
                  log_msg $2 'The system must forward audit records to the syslog service.' 
              else
                  log_msg $2 '系统必须能将审核记录转发给syslog服务。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38468)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must take appropriate action when the audit storage volume is full.' 
              else
                  log_msg $2 '当审计存储容量已满审计系统必须采取适当的行动。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38464)  if [ "$3" = "en" ]; then
                  log_msg $2 'The audit system must take appropriate action when there are disk errors on the audit storage volume.' 
              else
                  log_msg $2 '当审计存储卷发生硬盘错误时，审计系统必须采取适当的行动。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38462)  if [ "$3" = "en" ]; then
                  log_msg $2 'The package management tool must cryptographically verify the authenticity of all software packages during installation.' 
              else
                  log_msg $2 '在安装所有软件包时必须使用包管理工具加密验证软件包的真实性。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38460)  if [ "$3" = "en" ]; then
                  log_msg $2 'The NFS server must not have the all_squash option enabled.' 
              else
                  log_msg $2 'NFS服务中必须没有all_squash选项的开启。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38446)  if [ "$3" = "en" ]; then
                  log_msg $2 'The mail system must forward all mail for root to one or more system administrators.' 
              else
                  log_msg $2 '邮件系统必须转发所有的邮件到root或多个系统管理。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38445)  if [ "$3" = "en" ]; then
                  log_msg $2 'Audit log files must be group-owned by root.' 
              else
                  log_msg $2 '审计日志文件的组属主必须为root组。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38444)  if [ "$3" = "en" ]; then
                  log_msg $2 'The systems local IPv6 firewall must implement a deny-all, allow-by-exception policy for inboundpackets.' 
              else
                  log_msg $2 '系统的本地IPv6防火墙必须实现入站数据包的全部拒绝，允许按例外策略。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38438)  if [ "$3" = "en" ]; then
                  log_msg $2 'Auditing must be enabled at boot by setting a kernel parameter.' 
              else
                  log_msg $2 '审计必须在启动通过设置内核参数启用。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-38437)  if [ "$3" = "en" ]; then
                  log_msg $2 'Automated file system mounting tools must not be enabled unless needed.' 
              else
                  log_msg $2 '文件系统的自动挂载工具必须没有开启，除非必需。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-57569)  if [ "$3" = "en" ]; then
                  log_msg $2 'The noexec option must be added to the /tmp partition.' 
              else
                  log_msg $2 '不可执行(noexec)的选项必须添加到/tmp分区的挂载参数中。'
              fi
              if [ $2 -ne 0 ];then 
    ...
    V-58901)  if [ "$3" = "en" ]; then
                  log_msg $2 'The sudo command must require authentication.' 
              else
                  log_msg $2 '使用sudo命令必须进行验证。'
              fi
              if [ $2 -ne 0 ];then 
